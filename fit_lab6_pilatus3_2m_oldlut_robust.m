function out = fit_lab6_pilatus3_2m_oldlut_robust(matFile, userCfg)
%FIT_LAB6_PILATUS3_2M_OLDLUT_ROBUST
% Fast, accuracy-preserving PILATUS3 X 2M LaB6 pose fit.
%
% Adapted from the latest robust PILATUS 100K old+LUT implementation.
% The pose model and exact q-residual refinement are unchanged.  The detector
% layer is changed for a PILATUS3 X 2M raster: 172 um pixels, nominal
% 1679 rows x 1475 columns (or the transposed orientation), explicit
% inter-module gap masking, and Pilatus negative bad/gap pixel flags.
%
...
%
% Basic use:
%   out = fit_lab6_pilatus3_2m_oldlut_robust('lab6_pilatus3.mat');
%
% CPU/GPU control:
...
%   aps_detector_core.m         fallback q-phi output

if nargin < 1 || isempty(matFile)
    matFile = 'lab6_pilatus3.mat';
end
...
cfg = local_finalize_detector_cfg(cfg, size(Im));
...
tPhase = tic;
prep = local_prepare_image(Im, cfg, det);
...
fprintf('\n=== PILATUS3 X 2M fast old+LUT LaB6 pose fit ===\n');
fprintf('Data file: %s, variable: %s\n', matFile, imName);
fprintf('Image orientation: %s; image used for fit: %d rows x %d cols\n', ...
    orientInfo.name, size(Im,1), size(Im,2));
fprintf('Detector model: %s; active raster pixels after module-gap mask: %d / %d\n', ...
    det.name, nnz(det.activePixelMask), numel(det.activePixelMask));
...
profileRaw = local_profile_from_geom(Im, geom, cfg.profile.qEdges_Ainv, cfg.profile.trimPct, prep.validMask);
profileEnh = local_profile_from_geom(prep.enhancedForProfile, geom, cfg.profile.qEdges_Ainv, cfg.profile.trimPct, prep.validMask);
    ...
    try
        switch engine
            case 'fast'
                ...
                elseif exist('aps_detector_core', 'file') == 2
                    warning('tripol_qphi_fast not found; using aps_detector_core instead.');
                    TP = aps_detector_core('tripol', Im, geom, tripCfg);
                else
                    warning('No q-phi implementation was found; out.TP is empty.');
                end
            case 'aps'
                if exist('aps_detector_core', 'file') ~= 2
                    error('aps_detector_core.m was not found.');
                end
                TP = aps_detector_core('tripol', Im, geom, tripCfg);
                fprintf('Built q-phi output with aps_detector_core.\n');
            case {'none','off'}
                % Explicitly disabled.
            otherwise
...
% DECTRIS PILATUS3 X 2M nominal raster, MATLAB [rows cols].
cfg.detector.expectedSize_rc = [1679 1475];
cfg.detector.modulePixels_rc = [195 487];
cfg.detector.gapPixels_rc = [17 7];
cfg.detector.nModules_rc = [8 3];
cfg.detector.maskNominalModuleGaps = true;
cfg.detector.strictSize = false;
% PILATUS recorded images normally use -1 for gaps and -2 for bad pixels.
% These are always excluded.  Zero-count active pixels remain valid.
...
cfg.bounds = struct();
% beamX/beamY defaults are finalized after the image dimensions are known.
% Override these in userCfg whenever a rough beam intercept is available.
cfg.bounds.beamX_px = [];
cfg.bounds.beamY_px = [];
...
% ======================================================================
function cfg = local_finalize_detector_cfg(cfg, imSize)
% Fill detector-size-dependent search bounds only when the user did not set them.
nRows = imSize(1);
nCols = imSize(2);
if isempty(cfg.bounds.beamX_px)
    % Wide enough to permit a direct-beam intercept outside a tilted panel.
    cfg.bounds.beamX_px = [-0.60*nCols, 1.30*nCols];
end
if isempty(cfg.bounds.beamY_px)
    cfg.bounds.beamY_px = [-0.35*nRows, 1.35*nRows];
end
% Keep auto-expansion safety limits outside the initial detector-sized bounds.
xyLim = max(6000, 3*max(nRows,nCols));
cfg.search.absoluteLimits(1,:) = [-xyLim xyLim];
cfg.search.absoluteLimits(2,:) = [-xyLim xyLim];
end
...
expRC = cfg.detector.expectedSize_rc(:).';
isNative = isequal(imSize, expRC);
isSwapped = isequal(imSize, fliplr(expRC));
if ~(isNative || isSwapped)
    msg = sprintf(['PILATUS3 X 2M nominal image is %d x %d rows x cols ' ...
        '(or transposed). Loaded image is %d x %d. ' ...
        'Pose geometry will still use the loaded raster, but the nominal module-gap mask is disabled.'], ...
        expRC(1), expRC(2), imSize(1), imSize(2));
    if cfg.detector.strictSize
        error(msg);
    else
        warning(msg);
    end
...
gapRows = [];
gapCols = [];
if cfg.detector.maskNominalModuleGaps && (isNative || isSwapped)
    if isNative
        modRC = cfg.detector.modulePixels_rc;
        gapRC = cfg.detector.gapPixels_rc;
        nModRC = cfg.detector.nModules_rc;
    else
        modRC = fliplr(cfg.detector.modulePixels_rc);
        gapRC = fliplr(cfg.detector.gapPixels_rc);
        nModRC = fliplr(cfg.detector.nModules_rc);
    end
    gapRows = local_regular_gap_indices(imSize(1), modRC(1), gapRC(1), nModRC(1));
    gapCols = local_regular_gap_indices(imSize(2), modRC(2), gapRC(2), nModRC(2));
    active(gapRows,:) = false;
    active(:,gapCols) = false;
end

det.activePixelMask = active;
det.moduleGapRows = gapRows;
det.moduleGapCols = gapCols;
det.nominalSizeMatched = isNative || isSwapped;
end

% ======================================================================
function idx = local_regular_gap_indices(nPix, modulePix, gapPix, nModules)
idx = [];
if gapPix <= 0 || nModules <= 1
    return
end
pos = modulePix + 1;
for k = 1:(nModules-1)
    ii = pos:min(pos + gapPix - 1, nPix);
    idx = [idx ii]; %#ok<AGROW>
    pos = pos + gapPix + modulePix;
end
idx = unique(idx(idx >= 1 & idx <= nPix));
end
...
% ======================================================================
function prep = local_prepare_image(Im, cfg, det)
I0 = double(Im);
% PILATUS3 conventions: -1 = module gap, -2 = bad pixel.
% Preserve the proven ring-selection behavior: only positive counts are used.
% The explicit nominal gap mask also prevents gap-filled pixels entering the fit.
valid = det.activePixelMask & isfinite(I0) & I0 > 0;
if nnz(valid) < 1000
...
if cfg.ref.useCache && ~isempty(cfg.ref.cacheFile)
    try
        refCache = struct('key', cacheKey, 'ref', ref); %#ok<NASGU>
...
if cfg.search.autoExpandBounds && any(expandLo | expandHi)
    ...
    if any(newLb < lb) || any(newUb > ub)
        ...
        for j = 1:5
            if expandLo(j)
                parts{end+1} = sprintf('%s lower', names{j}); %#ok<AGROW>
            end
            if expandHi(j)
                parts{end+1} = sprintf('%s upper', names{j}); %#ok<AGROW>
            end
        end
...
for iLevel = 1:nLevels
    ...
    for k = 1:nSeedsHere
        ...
        Glocal = [Glocal; local_candidate_cloud(lbLoc, ubLoc, nPerSeed, skip)]; %#ok<AGROW>
    end
    ...
    allP = [allP; Glocal]; %#ok<AGROW>
    allScores = [allScores; scoresLocalApprox]; %#ok<AGROW>
    allSource = [allSource; repmat(uint8(4+iLevel), size(Glocal,1), 1)]; %#ok<AGROW>

    Pexact = [Pexact; PlevelExact]; %#ok<AGROW>
    Sexact = [Sexact; scoresLocalExact]; %#ok<AGROW>
    SexactSource = [SexactSource; repmat(uint8(4+iLevel), numel(scoresLocalExact), 1)]; %#ok<AGROW>
...
for j = 1:numel(orderedIdx)
    ...
    if isempty(idxSelected)
        ...
        if all(d >= minDistance)
            idxSelected(end+1,1) = idx; %#ok<AGROW>
        end
    end
...
for k = 1:K
    ...
    for cyc = 1:cfg.opt.reassignCycles
        ...
        [p, fval, info] = local_bounded_least_squares( ...
            resFcn, p, coarse.lb, coarse.ub, cfg.opt); %#ok<ASGLU>
...
% ======================================================================
function prof = local_profile_from_geom(Im, geom, qEdges, trimPct, validMask)
if nargin < 5 || isempty(validMask)
    validMask = true(size(Im));
end
qEdges = qEdges(:).';
qCenters = qEdges(1:end-1) + 0.5*diff(qEdges);
Icorr = Im .* geom.totalCorr;
valid = validMask & isfinite(Im) & isfinite(Icorr) & isfinite(geom.qMap) & ...
...
pts = out.scorePoints;
p = out.params;
figure('Name','PILATUS3 LaB6 signal processing audit: old+LUT', 'Color','w', 'Position',[50 80 1500 850]);
...
figure('Name','PILATUS3 LaB6 old+LUT pose result', 'Color','w', 'Position',[40 60 1550 900]);
tiledlayout(2,3,'TileSpacing','compact','Padding','compact');
...
y = out.profileEnhanced.profileRaw;
y = y(:);
if any(isfinite(y) & y > 0)
    y = y / max(y(isfinite(y)) + eps);
end
plot(ref.qGrid_Ainv, ref.profile / max(ref.profile + eps), 'LineWidth',1.2); hold on
plot(q, y, '-', 'MarkerSize',8)
for k = 1:numel(levels)
    xline(levels(k), ':');
end
xlim([min(q) max(q)]); ylim([0 1.05]); grid on
xlabel('Q (A^{-1})'); ylabel('normalized')
title('1D profile check: enhanced image collapsed by fitted pose')
legend({'LaB6 reference','observed enhanced profile'}, 'Location','northeast')
